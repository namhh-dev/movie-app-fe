...
export default function MovieForm({ state, updateState, optionState ,updateOptionState }) {
    ...
    return (
        <div className='border-[2px] h-[1000px] px-1 rounded-md overflow-scroll scrollbar-hidden'>
            ...
            <ImageUpload state={state} updateState={updateState} />
            ...
            <div className="mb-4">
                <label htmlFor="content" className="block mb-2 text-md font-bold text-gray-900 dark:text-white">Content</label>
                <textarea
                    onChange={e => updateState("content", e.target.value)}
                    value={state.content}
                    rows={10}
                    id="content"
                    className="bg-white border border-gray-200 placeholder-slate-300 text-black text-sm rounded-md block w-full p-2"
...
const ImageUpload = ({ state, updateState }) => (
    <div className='pr-2'>
        <div className={`flex gap-4 items-center h-[195px] mr-2`}>
            ...
            <div className="flex-1 border h-[195px] p-1 rounded-md flex items-center justify-center" style={{ flex: '0 0 23%' }}> 
                ...
                ? 
                    ...
                    :
                    <p className='text-gray-700 font-medium'>Poster Image</p>
                }
            </div>
        </div>
...
const ImagePreview = ({ url, alt }) => (
    <div className="w-[125px]">
        <img src={url} alt={alt} className="max-w-full h-auto border rounded shadow" onError={(e) => e.target.style.display = 'none'} />
    </div>
);
