          ...
          <ToggleSection title="Danh sách tập">
            <div className="flex flex-wrap gap-3">
              {movie.Episodes.map((episode) => (
                <Link key={episode.ep_id} to={`/movie/${movie.mov_id}/ep/${episode.ep_id}`} className="py-1 bg-gray-400 text-center rounded-md w-[80px]">
                  {episode.ep_name}
                </Link>
              ))}
            </div>
  ...
  <div className="relative w-[250px] h-[400px]">
    ...
    <div className="absolute bottom-0 w-full px-2 py-1 bg-transparent backdrop-blur-lg">
      <div className="flex justify-between">
        <button className="py-1 px-4 bg-purple-900 text-white rounded-md">Xem trailer</button>
        <Link to={`/movie/${movie.mov_id}/ep/${movie.Episodes[0].ep_id}`} className="py-1 px-4 bg-red-800 text-white rounded-md">
          Xem phim
        </Link>
      </div>
    </div>
